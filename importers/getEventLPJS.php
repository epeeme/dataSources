...
require '../scrapers/lpjsParser/lpjsParser.php';
...
// COMPETITION DATA 
$ageArray = array(10,8,3,5,9,7,4,1);
$eventID = 6;
$eventDate = '2019-02-09';
$eventYear = 2019;
$eventURL = "https://leonpauljuniorseries.co.uk/results/view/LPJScorepee19/";
// ENDS
...
// Now clean out the lpjs holding file so it's ready to take the new data

$s = $db -> prepare('DELETE FROM lpjs');
$db -> execute($s);

// Grab all the data from the source URL and place it in the 
// now empty lpjs table.

// example;
// https://leonpauljuniorseries.co.uk/results/view/LPJScorepee19/
...
$resultsBody = stristr(stristr($resultsBody, '<ul class="dots lp-bullet">'), '</ul>', true);
...
for ($i=0; $i < count($tableRows); $i++ ) {
    ...
    if ($ageArray[$i] > 0) {
        ...
        $lpjsResultsURL = get_istring_between($tableRows[$i], '<a href="', '">');
        $e = new lpjs\LPJSParser("https:".$lpjsResultsURL);
        $allResults = $e->getAllResults();
    
        $numberOfEntries = 0;

        foreach ($allResults as $value) {
            $s  = $db -> prepare('INSERT INTO lpjs (eventID, eventYear, eventCat, fencerFirstname, fencerSurname, fencerClub, eventPosition, lpjsPoints) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
            $db -> bind($s, 1, $eventID);      
...
$s  = $db -> prepare('UPDATE lpjs SET fencerID = (SELECT fencers.ID FROM fencers WHERE fencers.fencerFirstname = lpjs.fencerFirstname AND fencers.fencerSurname = lpjs.fencerSurname LIMIT 0,1)');
$db -> execute($s);

// And last but not least, handle the club data

$s  = $db -> prepare('UPDATE lpjs SET fencerClubID = (SELECT clubsAlias.clubID FROM clubsAlias WHERE clubsAlias.alias = lpjs.fencerClub LIMIT 0,1)');
$db -> execute($s);    

// And to round off, display the SQL to be run in the console to update
// and commit the manual changes.

echo "<P>Make any changes to fencers names and run this<BR><CODE>UPDATE lpjs SET fencerID = (SELECT fencers.ID FROM fencers WHERE fencers.fencerFirstname = lpjs.fencerFirstname AND fencers.fencerSurname = lpjs.fencerSurname LIMIT 0,1)</CODE></P>";
echo '<hr>';
echo '<P>Add in new fencers<BR><CODE>INSERT INTO fencers (fencerFirstname, fencerSurname, fencerFullname) SELECT lpjs.fencerFirstname, lpjs.fencerSurname, CONCAT(lpjs.fencerFirstname," ",lpjs.fencerSurname) FROM lpjs WHERE fencerID = 0</CODE></P>';
echo '<P>Alt with a sex specific & ranking inclusiion<BR><CODE>INSERT INTO fencers (fencerFirstname, fencerSurname, fencerFullname, sex, efr) SELECT lpjs.fencerFirstname, lpjs.fencerSurname, CONCAT(lpjs.fencerFirstname," ",lpjs.fencerSurname),"Male", 1 FROM lpjs WHERE fencerID = 0</CODE></P>';
echo '<hr>';
echo '<P>Check clubs and add any new aliases<BR><CODE>UPDATE lpjs SET fencerClubID = (SELECT clubsAlias.clubID FROM clubsAlias WHERE clubsAlias.alias = lpjs.fencerClub LIMIT 0,1)</CODE></P>';
echo '<hr>';
echo '<P>Make live<BR><CODE>INSERT INTO results (eventID, dateID, eventCat, fencerID, fencerClubID, eventPosition, lpjsPoints) SELECT lpjs.eventID, eventDates.ID , eventCat, fencerID, fencerClubID, eventPosition, lpjsPoints FROM lpjs LEFT OUTER JOIN eventDates ON year = eventYear AND eventDates.eventID = lpjs.eventID</CODE></P>';
echo '<hr>';
